...
if ($_FILES['ma_pp']['error']) {
  switch ($_FILES['ma_pp']['error']){
  ...
  $name = $_FILES['ma_pp']['name'];
  $nom = $_FILES['ma_pp']['tmp_name'];
  $nomdestination = './document_user/pp_user/'.$name.'';
  ...
  $delete = $bdd->query('DELETE  FROM documents WHERE ID_utilisateurs_Possede = "'.$id_user.'" AND Code_document__CV_ou_LM_ = "PP"');
